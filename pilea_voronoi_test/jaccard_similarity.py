...
def overlap_test(G, seeds, bounded_regions, type = 'dot'):
    ...
    for i in range(L):
        # diff_1 =  Polygon(G.graph['faces_passed'][i]).buffer(0).difference(Polygon(bounded_regions[passed_index[i]]))
        # diff_2 =  Polygon(bounded_regions[passed_index[i]]).difference(Polygon(G.graph['faces_passed'][i]).buffer(0))
        # difference_geom.append(diff_1)
        # difference_geom.append(diff_2)
