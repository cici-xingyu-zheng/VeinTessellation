...
def plot_random_rounds(mean_angle_error, mean_dist_error, rst_summary, deposit_folder):
    ...
    'histplot of random mean errors; not used in the later analysis'
...
def plot_testable(G, G_dual, deposit_folder):
    ...
    fig.savefig(f'{deposit_folder}/testable_pairs.pdf')

    plt.show()   


def plot_overlap_diff(G, deposit_folder, bounded_regions, seeds, single_dot, type = 'dot'):

    'this is the new function for visualizing similarity test results'
    ...
    fig.savefig(f'{deposit_folder}/area_overlap_difference.pdf')
